...
import { 
  ...
  SlidersHorizontal, RefreshCcw, Tags, Briefcase, 
  LayoutDashboard, BarChart3, Download, Bell, TrendingUp, DollarSign, Activity, FileSpreadsheet,
  ShoppingCart, Users, Clock, CheckCircle, Truck, XCircle, CheckSquare, Calendar, Shield, Palette, Ruler,
  UserPlus, Check, User as UserIcon, Loader2, Info, Upload, FolderPlus, List
} from 'lucide-react';
...
type SortField = 'name' | 'price' | 'category' | 'stock' | 'rating';
type SortDirection = 'asc' | 'desc';
type StockStatus = 'all' | 'in_stock' | 'low_stock' | 'out_of_stock';
type ActiveTab = 'dashboard' | 'products' | 'categories' | 'brands' | 'orders' | 'customers' | 'tasks' | 'admins';
type Permission = 'dashboard' | 'products' | 'categories' | 'brands' | 'orders' | 'customers' | 'tasks' | 'admins';
...
interface Order {
  ...
  customerName: string; // Snapshot name
  date: string;
  total: number;
...
const MOCK_CUSTOMERS: Customer[] = [
  { id: 'CUST-001', name: 'أحمد محمد', email: 'ahmed@example.com', joinDate: '2025-01-15', totalOrders: 5, totalSpent: 2450 },
  { id: 'CUST-002', name: 'سارة علي', email: 'sara@example.com', joinDate: '2025-03-22', totalOrders: 12, totalSpent: 8500 },
  { id: 'CUST-003', name: 'خالد عمر', email: 'khaled@example.com', joinDate: '2025-06-10', totalOrders: 2, totalSpent: 500 },
  { id: 'CUST-004', name: 'منى يوسف', email: 'mona@example.com', joinDate: '2025-08-05', totalOrders: 1, totalSpent: 85 },
];

const MOCK_ORDERS: Order[] = [
  { id: 'ORD-001', customerId: 'CUST-001', customerName: 'أحمد محمد', date: '2025-10-25', total: 450, status: 'delivered', itemsCount: 2 },
  { id: 'ORD-002', customerId: 'CUST-002', customerName: 'سارة علي', date: '2025-10-26', total: 1200, status: 'shipped', itemsCount: 4 },
  { id: 'ORD-003', customerId: 'CUST-003', customerName: 'خالد عمر', date: '2025-10-27', total: 299, status: 'processing', itemsCount: 1 },
  { id: 'ORD-004', customerId: 'CUST-004', customerName: 'منى يوسف', date: '2025-10-27', total: 85, status: 'pending', itemsCount: 1 },
  { id: 'ORD-005', customerId: 'CUST-001', customerName: 'أحمد محمد', date: '2025-10-24', total: 3200, status: 'cancelled', itemsCount: 1 },
];

const MOCK_TASKS: Task[] = [
  { id: 1, text: 'مراجعة تقارير المبيعات الشهرية', completed: false, dueDate: '2025-11-15', priority: 'high' },
  { id: 2, text: 'التواصل مع مورد الإلكترونيات', completed: true, dueDate: '2025-11-10', priority: 'medium' },
  { id: 3, text: 'تحديث بنرات الصفحة الرئيسية', completed: false, dueDate: '2025-11-20', priority: 'low' },
];
...
const MOCK_ADMINS: AdminUser[] = [
  { 
    ...
    permissions: ['dashboard', 'products', 'categories', 'brands', 'orders', 'customers', 'tasks', 'admins'], 
    lastLogin: '2025-10-27 09:30',
    isSuperAdmin: true
  },
  { 
    id: 2, 
...
const AVAILABLE_PERMISSIONS: { key: Permission; label: string }[] = [
  ...
  { key: 'brands', label: 'إدارة العلامات التجارية' },
  { key: 'orders', label: 'إدارة الطلبات' },
  { key: 'customers', label: 'إدارة العملاء' },
  { key: 'tasks', label: 'المهام والملاحظات' },
  { key: 'admins', label: 'إدارة المشرفين' },
];
...
const OrderStatusBadge: React.FC<{ status: Order['status'] }> = ({ status }) => {
  const styles = {
    ...
    processing: { bg: 'bg-blue-100', text: 'text-blue-700', icon: Activity, label: 'قيد التجهيز' },
    shipped: { bg: 'bg-indigo-100', text: 'text-indigo-700', icon: Truck, label: 'تم الشحن' },
    delivered: { bg: 'bg-green-100', text: 'text-green-700', icon: CheckCircle, label: 'تم التوصيل' },
    cancelled: { bg: 'bg-red-100', text: 'text-red-700', icon: XCircle, label: 'ملغي' },
  };
...
const AdminPage: React.FC<AdminPageProps> = ({ 
  ...
  const [orders, setOrders] = useState<Order[]>(() => loadFromStorage('orders', MOCK_ORDERS));

  // Customers State (Persisted)
  const [customers, setCustomers] = useState<Customer[]>(() => loadFromStorage('customers', MOCK_CUSTOMERS));
  const [isCustomerModalOpen, setIsCustomerModalOpen] = useState(false);
  const [editingCustomer, setEditingCustomer] = useState<Customer | null>(null);
  const [customerFormData, setCustomerFormData] = useState({ name: '', email: '' });

  // Tasks State (Persisted)
  const [tasks, setTasks] = useState<Task[]>(() => loadFromStorage('tasks', MOCK_TASKS));
  const [newTaskText, setNewTaskText] = useState('');
  const [newTaskDate, setNewTaskDate] = useState('');
  const [newTaskPriority, setNewTaskPriority] = useState<Task['priority']>('medium');

  // Admins State (Persisted)
  const [admins, setAdmins] = useState<AdminUser[]>(() => loadFromStorage('admins', MOCK_ADMINS));
  ...
  const [deleteCustomerConfirmation, setDeleteCustomerConfirmation] = useState<string | null>(null);
  const [deleteAdminConfirmation, setDeleteAdminConfirmation] = useState<number | null>(null);
  ...
  // Trigger low stock notification on mount
  useEffect(() => {
    const lowStockItems = products.filter(p => p.stock <= 5);
    const outOfStockItems = lowStockItems.filter(p => p.stock === 0);

    if (outOfStockItems.length > 0) {
        const timer = setTimeout(() => {
            showToast(`تنبيه: نفذت كمية ${outOfStockItems.length} منتجات! يرجى إعادة التخزين.`, 'error');
        }, 800);
        return () => clearTimeout(timer);
    } else if (lowStockItems.length > 0) {
        const timer = setTimeout(() => {
            showToast(`تنبيه: مخزون ${lowStockItems.length} منتجات منخفض.`, 'info');
        }, 800);
        return () => clearTimeout(timer);
    }
  }, []); // Run once on mount
  ...
  const handleOpenAddAdmin = () => {
    ...
    setAdminFormData({ name: '', username: '', role: '', permissions: [] });
  ...
  const handleAdminSubmit = (e: React.FormEvent) => {
    ...
    simulateLoading(() => {
      if (editingAdmin) {
        ...
        showToast('تم إضافة مشرف جديد بنجاح');
      }
  ...
  const handleConfirmDeleteAdmin = () => {
    if (deleteAdminConfirmation !== null) {
      simulateLoading(() => {
        setAdmins(prev => prev.filter(a => a.id !== deleteAdminConfirmation));
  ...
  const handleUpdateOrderStatus = (orderId: string, newStatus: Order['status']) => {
    simulateLoading(() => {
        ...
        showToast(`تم تغيير حالة الطلب #${orderId}`);
    });
  };

  const handleConfirmDeleteOrder = () => {
    if (deleteOrderConfirmation !== null) {
      simulateLoading(() => {
        setOrders(prev => prev.filter(o => o.id !== deleteOrderConfirmation));
  ...
  const handleOpenAddCustomer = () => {
    ...
    setCustomerFormData({ name: '', email: '' });
    setIsCustomerModalOpen(true);
  };

  const handleOpenEditCustomer = (customer: Customer) => {
    setEditingCustomer(customer);
    setCustomerFormData({ name: customer.name, email: customer.email });
    setIsCustomerModalOpen(true);
  };
    ...
    simulateLoading(() => {
      if (editingCustomer) {
        setCustomers(prev => prev.map(c => 
          c.id === editingCustomer.id ? { ...c, name: customerFormData.name, email: customerFormData.email } : c
        ));
  ...
  const handleConfirmDeleteCustomer = () => {
    if (deleteCustomerConfirmation !== null) {
      simulateLoading(() => {
        setCustomers(prev => prev.filter(c => c.id !== deleteCustomerConfirmation));
        setDeleteCustomerConfirmation(null);
  ...
  const handleProductSubmit = (e: React.FormEvent) => {
    ...
    // Handle new category
    if (isNewCategoryMode && productFormData.category && !categories.includes(productFormData.category)) {
        onAddCategory(productFormData.category);
    }
  ...
  const handleBrandSubmit = (e: React.FormEvent) => {
    ...
    simulateLoading(() => {
        if (editingBrand) {
            ...
            showToast('تم تحديث العلامة التجارية');
        } else {
            onAddBrand(brandFormData);
            showToast('تم إضافة علامة تجارية جديدة');
        }
  ...
  const handleConfirmDeleteBrand = () => {
    if (deleteBrandConfirmation !== null) {
        simulateLoading(() => {
            ...
            showToast('تم حذف العلامة التجارية');
        });
    }
  };
  ...
  // Helper to remove attribute from string
  const handleRemoveAttribute = (type: 'colors' | 'sizes', valueToRemove: string) => {
    const input = type === 'colors' ? colorsInput : sizesInput;
    const setInput = type === 'colors' ? setColorsInput : setSizesInput;
    
    const items = input.split(',').map(i => i.trim()).filter(i => i);
    const newItems = items.filter(i => i !== valueToRemove);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in relative">
      ...
      {isLoading && (
        <div className="fixed top-0 left-0 right-0 h-1 bg-blue-100 z-[100] overflow-hidden">
            <div className="h-full bg-primary animate-progress origin-left w-full shadow-[0_0_10px_rgba(37,99,235,0.5)]"></div>
             <style>{`
                @keyframes progress {
        ...
        <div className="flex items-center gap-3">
             ...
             <div className="relative">
                ...
                >
                    ...
                    {(lowStockProducts.length > 0 || pendingOrders > 0 || pendingTasksCount > 0) && (
                ...
                {showNotifications && (
                    <div className="absolute left-0 mt-2 w-72 bg-white rounded-xl shadow-xl border border-gray-100 z-50 overflow-hidden animate-fade-in-up">
                        <div className="p-3 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                            ...
                            <span className="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full">{lowStockProducts.length + pendingOrders + pendingTasksCount}</span>
                        </div>
                        <div className="max-h-64 overflow-y-auto">
                            {lowStockProducts.length === 0 && pendingOrders === 0 && pendingTasksCount === 0 ? (
                                ...
                                <>
                                  {pendingTasksCount > 0 && (
                                     <div className="p-3 hover:bg-gray-50 transition-colors border-b border-gray-50 flex gap-3 cursor-pointer" onClick={() => setActiveTab('tasks')}>
                                          <div className="w-10 h-10 bg-indigo-50 rounded-lg flex items-center justify-center flex-shrink-0 text-indigo-500">
                                              <CheckSquare className="w-5 h-5" />
                                          </div>
                                          <div>
                                              <p className="text-sm font-bold text-gray-800">مهام معلقة</p>
                                              <p className="text-xs text-gray-500">لديك {pendingTasksCount} مهام غير مكتملة</p>
                                          </div>
                                      </div>
                                  )}
                                  {pendingOrders > 0 && (
                                     <div className="p-3 hover:bg-gray-50 transition-colors border-b border-gray-50 flex gap-3 cursor-pointer" onClick={() => setActiveTab('orders')}>
                                          <div className="w-10 h-10 bg-amber-50 rounded-lg flex items-center justify-center flex-shrink-0 text-amber-500">
                                              <Clock className="w-5 h-5" />
                                          </div>
                                          <div>
                                              <p className="text-sm font-bold text-gray-800">طلبات جديدة</p>
                                              <p className="text-xs text-gray-500">لديك {pendingOrders} طلبات قيد الانتظار</p>
                                          </div>
                                      </div>
                                  )}
                                  {lowStockProducts.map(p => (
                                    <div key={p.id} className="p-3 hover:bg-gray-50 transition-colors border-b border-gray-50 last:border-0 flex gap-3 cursor-pointer" onClick={() => { setActiveTab('products'); setSearchTerm(p.name); }}>
      ...
      <div className="mb-8 border-b border-gray-200 overflow-x-auto">
        <nav className="flex -mb-px gap-8 min-w-max px-2" aria-label="Tabs">
          ...
          >
             ...
             الطلبات
             {pendingOrders > 0 && <span className="bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-full animate-bounce">{pendingOrders}</span>}
          </button>
          <button
          ...
          >
            ...
            المهام
            {pendingTasksCount > 0 && <span className="bg-indigo-500 text-white text-[10px] px-1.5 py-0.5 rounded-full">{pendingTasksCount}</span>}
          </button>
           <button
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4 hover:shadow-md transition-shadow">
                      ...
                      <div>
                          ...
                          <p className="text-xl font-black text-gray-900 dir-ltr">{totalSales.toLocaleString()} SAR</p>
                      </div>
                  </div>
                  <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4 hover:shadow-md transition-shadow">
                   ...
                   <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4 hover:shadow-md transition-shadow">
                      ...
                      <div>
                          <p className="text-xs text-gray-500 font-bold mb-1">العملاء</p>
                      ...
                      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                              <button 
                                ...
                                className="p-4 bg-gray-50 rounded-xl hover:bg-primary hover:text-white transition-all text-center flex flex-col items-center gap-2 group shadow-sm active:scale-95"
                              >
                               ...
                               <button 
                                onClick={handleExportCSV}
                                className="p-4 bg-gray-50 rounded-xl hover:bg-green-600 hover:text-white transition-all text-center flex flex-col items-center gap-2 group shadow-sm active:scale-95"
                              >
      ...
      {activeTab === 'admins' && (
         ...
         <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden animate-fade-in-up">
            <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <h3 className="font-bold text-gray-900">إدارة المشرفين وصلاحيات الوصول</h3>
                <button 
                  onClick={handleOpenAddAdmin}
                  className="flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-secondary transition-colors shadow-sm active:scale-95"
                >
                    <Shield className="w-4 h-4" />
                    إضافة مشرف جديد
                </button>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-right">
                    <thead className="bg-gray-50 border-b border-gray-200">
                        <tr>
                            <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase">الاسم / اسم المستخدم</th>
                            <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase">الدور</th>
                            <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase">الصلاحيات</th>
                            <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase">آخر تسجيل دخول</th>
                            <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase text-left">إجراءات</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                        {admins.map((admin) => (
                            <tr key={admin.id} className="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                    {admin.isSuperAdmin ? (
                                        <span className="flex items-center gap-1.5 text-purple-600 font-bold bg-purple-50 px-2 py-1 rounded-full w-fit">
                                            <Shield className="w-3.5 h-3.5 fill-current" />
                                            {admin.role}
                                        </span>
                                    ) : (
                                        <span className="font-medium bg-gray-100 px-2 py-1 rounded-md text-xs">{admin.role}</span>
                                    )}
                                </td>
                                <td className="px-6 py-4">
                                    <div className="flex flex-wrap gap-1 max-w-xs">
                                        {admin.permissions.includes('dashboard') && admin.permissions.length === AVAILABLE_PERMISSIONS.length ? (
                                            <span className="text-xs bg-green-50 text-green-700 px-2 py-1 rounded border border-green-100">كل الصلاحيات</span>
                                        ) : (
                                            admin.permissions.slice(0, 3).map(p => (
                                                <span key={p} className="text-[10px] bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded border border-blue-100">
                                                    {AVAILABLE_PERMISSIONS.find(ap => ap.key === p)?.label || p}
                                                </span>
                                            ))
                                        )}
                                        {admin.permissions.length > 3 && !(admin.permissions.includes('dashboard') && admin.permissions.length === AVAILABLE_PERMISSIONS.length) && (
                                            <span className="text-[10px] text-gray-500 px-1">+{admin.permissions.length - 3}</span>
                                        )}
                                    </div>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dir-ltr text-right">{admin.lastLogin}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-left">
                                    <div className="flex items-center justify-end gap-2">
                                        <button 
                                            ...
                                            title={admin.isSuperAdmin ? "لا يمكن تعديل المدير العام" : "تعديل الصلاحيات"}
                                        >
                                            <Edit2 className="w-4 h-4" />
                                        </button>
                                        <button 
                                            onClick={() => setDeleteAdminConfirmation(admin.id)}
                                            className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"
                                            disabled={admin.isSuperAdmin}
                                            title="حذف"
                                        >
                        ...
                        <div className="relative flex gap-2">
                            {isNewCategoryMode ? (
                                <input
                                    ...
                                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none animate-fade-in"
                            ...
                            <button
                                ...
                                onClick={() => {
                                    ...
                                    if (!isNewCategoryMode) setProductFormData({...productFormData, category: ''});
                                }}
                                className={`px-3 rounded-lg border flex items-center justify-center transition-colors ${isNewCategoryMode ? 'bg-gray-100 border-gray-300 text-gray-600 hover:bg-gray-200' : 'bg-primary text-white border-primary hover:bg-secondary'}`}
                                title={isNewCategoryMode ? "اختر من القائمة" : "إضافة تصنيف جديد"}
                                disabled={isLoading}
                            >
                                {isNewCategoryMode ? <List className="w-5 h-5" /> : <FolderPlus className="w-5 h-5" />}
                            </button>
                        </div>
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mb-3">
                        ...
                        <button 
                            ...
                            disabled={isLoading}
                            className="px-3 py-2 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 flex items-center gap-1 disabled:opacity-50"
      ...
      {isCustomerModalOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
             <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                 <div className="fixed inset-0 transition-opacity" onClick={() => !isLoading && setIsCustomerModalOpen(false)}>
                     <div className="absolute inset-0 bg-gray-900/75 backdrop-blur-sm"></div>
                 </div>
                 <div className="inline-block align-bottom bg-white rounded-2xl text-right overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md w-full">
                     <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                         <div className="flex justify-between items-center mb-5">
                             ...
                             <button onClick={() => setIsCustomerModalOpen(false)} disabled={isLoading} className="text-gray-400 hover:text-gray-500 disabled:opacity-50">
                                 <X className="w-5 h-5" />
                             </button>
                         </div>
                 ...
                 <div className="inline-block align-bottom bg-white rounded-2xl text-right overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
                     <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                         <div className="flex justify-between items-center mb-5">
                             ...
                             <button onClick={() => setIsAdminModalOpen(false)} disabled={isLoading} className="text-gray-400 hover:text-gray-500 disabled:opacity-50">
                                 <X className="w-5 h-5" />
                             </button>
                         </div>
                                 ...
                                 <div className="grid grid-cols-2 gap-4">
                                     ...
                                     <div>
                                         ...
                                         <input
                                             type="text"
                                             required
                                             className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none dir-ltr"
                                 ...
                                 <div>
                                     ...
                                     <div className="grid grid-cols-2 gap-3 bg-gray-50 p-3 rounded-lg border border-gray-100">
                                         {AVAILABLE_PERMISSIONS.map((perm) => (
                                             <label key={perm.key} className="flex items-center gap-2 cursor-pointer p-2 hover:bg-white rounded transition-colors">
                                                 <input 
                                                     type="checkbox"
      ...
      {(deleteConfirmationId !== null || deleteCategoryConfirmation !== null || deleteBrandConfirmation !== null || deleteOrderConfirmation !== null || deleteCustomerConfirmation !== null || deleteAdminConfirmation !== null) && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
            <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                    ...
                    <div className="sm:flex sm:items-start">
                        ...
                        <div className="mt-3 text-center sm:mt-0 sm:mr-4 sm:text-right">
                            <h3 className="text-lg leading-6 font-medium text-gray-900">
                                {deleteConfirmationId ? 'حذف المنتج' : deleteCategoryConfirmation ? 'حذف التصنيف' : deleteBrandConfirmation ? 'حذف العلامة التجارية' : deleteOrderConfirmation ? 'حذف الطلب' : deleteCustomerConfirmation ? 'حذف العميل' : 'حذف المشرف'}
                            </h3>
                            <div className="mt-2">
                                <p className="text-sm text-gray-500">
                                            ...
                                            : deleteBrandConfirmation
                                                ...
                                                : deleteOrderConfirmation
                                                    ...
                                                    : deleteCustomerConfirmation
                                                        ? 'هل أنت متأكد من حذف هذا العميل وجميع بياناته؟'
                                                        : 'هل أنت متأكد من حذف حساب المشرف هذا؟'
                                    }
                                </p>
                            </div>
                        </div>
                ...
                <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <button
                        ...
                        onClick={() => {
                            ...
                            else if (deleteCustomerConfirmation) handleConfirmDeleteCustomer();
