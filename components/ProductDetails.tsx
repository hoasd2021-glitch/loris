    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
        ...
        <div className="space-y-4">
          <div className="aspect-square bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100 relative group">
            ...
            <button 
                ...
                className="absolute top-4 right-4 p-3 rounded-full bg-white/90 backdrop-blur shadow-sm hover:bg-white transition-all transform hover:scale-110"
            >
        ...
        <div className="flex flex-col">
           <div className="mb-6">
                {product.brand && (
                    <span className="text-sm font-bold text-primary bg-primary/5 px-3 py-1 rounded-full mb-3 inline-block">
                        {product.brand}
                    </span>
                )}
                ...
                <div className="flex items-center gap-4 text-sm">
                    <div className="flex items-center gap-1 text-amber-400">
                        ...
                        <span className="text-gray-400 font-normal">({reviewsCount} تقييم)</span>
                    </div>
           ...
           <div className="text-3xl font-bold text-gray-900 mb-8 dir-ltr flex items-center justify-end sm:justify-start gap-1">
           ...
           <div className="space-y-6 mb-8 flex-1">
                ...
                {product.colors && product.colors.length > 0 && (
                    <div>
                        <span className="block text-sm font-bold text-gray-900 mb-3">اللون المختار: <span className="text-primary font-normal">{selectedColor}</span></span>
                ...
                {product.sizes && product.sizes.length > 0 && (
                    <div>
                        <span className="block text-sm font-bold text-gray-900 mb-3">المقاس: <span className="text-primary font-normal">{selectedSize}</span></span>
                        <div className="flex flex-wrap gap-3">
           ...
           {/* Actions */}
           <div className="flex items-center gap-4 py-6 border-t border-gray-100 mt-auto">
                <div className="flex items-center border border-gray-200 rounded-xl h-12 bg-white">
                    <button 
                        onClick={() => setQuantity(Math.max(1, quantity - 1))}
                        className="w-12 h-full flex items-center justify-center text-gray-500 hover:text-primary transition-colors text-lg"
                    >
                        -
                    </button>
                ...
                <button className="h-12 w-12 rounded-xl border border-gray-200 flex items-center justify-center text-gray-500 hover:text-primary hover:bg-blue-50 transition-colors">
      ...
      <div className="mb-16">
          <div className="border-b border-gray-200 mb-8">
              <nav className="flex gap-8">
                  <button
                    ...
                    className={`pb-4 text-sm font-bold border-b-2 transition-colors ${activeTab === 'description' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
                  >
                      تفاصيل المنتج
                  </button>
                  <button
                    onClick={() => setActiveTab('reviews')}
                    className={`pb-4 text-sm font-bold border-b-2 transition-colors ${activeTab === 'reviews' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
                  >
          ...
          <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 min-h-[200px]">
              {activeTab === 'description' ? (
                  <div className="prose max-w-none text-gray-600 leading-loose whitespace-pre-line">
                          ...
                          {currentUser ? (
                              <form onSubmit={handleSubmitReview} className="space-y-4">
                                  <div className="flex items-center gap-2 mb-2">
                                      <span className="text-sm font-medium text-gray-600 ml-2">تقييمك:</span>
                                      {[1, 2, 3, 4, 5].map((star) => (
                                          <button
                                              key={star}
                                              type="button"
                                  ...
                                  <div className="relative">
                                      <textarea
                                          ...
                                          rows={3}
                                          className="w-full p-4 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none resize-none bg-white"
                                          required
                                      />
                                      <button 
                                          type="submit"
                              ...
                              <div className="text-center py-6">
                                  <p className="text-gray-500 mb-4">يرجى تسجيل الدخول لتتمكن من إضافة تقييم.</p>
                                  <button 
                                      onClick={onLoginClick}
                                      className="bg-gray-900 text-white px-6 py-2 rounded-full font-bold text-sm hover:bg-gray-800 transition-colors shadow-lg"
                                  >
                                      تسجيل الدخول
                                  </button>
                              </div>
                          ...
                          {(!product.reviews || product.reviews.length === 0) ? (
                              ...
                              product.reviews.map((review) => (
                                  <div key={review.id} className="flex gap-4 p-4 rounded-2xl hover:bg-gray-50 transition-colors border border-transparent hover:border-gray-100">
                                      <div className="w-12 h-12 rounded-full bg-blue-50 flex items-center justify-center font-bold text-primary border border-blue-100 flex-shrink-0">
                      ...
                      >
                           ...
                           <div className="p-4">
                                ...
                                <p className="text-primary font-bold mt-2 dir-ltr">
                                    {currencySymbol} {currency === 'SAR' ? prod.price : (prod.price / exchangeRate).toFixed(2)}
                                </p>
                           </div>
                      </div>
                 ))}
